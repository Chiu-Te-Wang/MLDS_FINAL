...
def main():
    ...
    parser.add_argument('--output', type=str, default='./prediction.csv',
                        help='output path') 
    parser.add_argument('--resume_model', type=str, default='BEST',
                        help='index of resume model')
    parser.add_argument('--resume_model_certain', type=str, default='',
                        help='certain model to resume')
...
def infer(args):
    ...
    # ===================================
    choices = ['a', 'b', 'c', 'd', 'e']
    if args.resume_model_certain == '':
        if args.resume_model == "BEST":
            checkpoint = tf.train.latest_checkpoint(args.save_dir)
        else:
            checkpoint = args.save_dir+"model.ckpt-{}".format(args.resume_model)
    else:
        checkpoint = args.save_dir+args.resume_model_certain
...
if __name__ == '__main__':
    #python3 inference.py --save_dir=./save/ --test_file=./data/testing_data.csv --output=pred.csv --resume_model=6000
    #python3 inference.py --save_dir=./save/ --test_file=./data/testing_data.csv --output=pred.csv --resume_model_certain=model_GL0.ckpt-50410
