...
class TextLoader():
    ...
    def preprocess(self, input_files, vocab_file, data_file):
        ...
        self.num_batches = self.num_data // self.batch_size -1
    ...
    def next_batch(self, shuffle=True):
        if self.pointer + self.batch_size > self.num_data:
            # self.pointer = 0
            self.pointer = self.batch_size*valid_num
        
        # if self.pointer == 0 and shuffle:
        if self.pointer == self.batch_size*valid_num and shuffle:
    ...
    def reset_batch_pointer(self):
        # self.pointer = 0
        self.pointer = self.batch_size*valid_num
    def get_first_batch_as_valid(self):
        xList = list()
        yList = list()
        startFromPointer = 0
        for i in range(valid_num):
            x, y = self._data[startFromPointer:startFromPointer+self.batch_size, 0:-1], self._data[startFromPointer:startFromPointer+self.batch_size, 1: ]
            startFromPointer += self.batch_size
            xList.append(x)
            yList.append(y)
        return xList, yList
